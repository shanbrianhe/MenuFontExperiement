...
#full model
ATE_model_full <- lm(Item_Price ~ Serif * factor(Font_family) * factor(item_number) + Income + Age + Male, data = d)
...
obs.cace <- coef(ATE_model_long)[2]

plot(density(simulation.sharp.null))
abline(v=obs.cace)
mean(obs.cace >= simulation.sharp.null)
```
